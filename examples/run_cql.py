...
def train(args=get_args()):
    ...
    # create policy model
    actor_backbone = MLP(input_dim=np.prod(args.next_obs_shape), hidden_dims=args.hidden_dims)
    # TODO: ask if action dim is needed or not
    # policy training uses only observations but model was defined with action dim
    critic1_backbone = MLP(input_dim=np.prod(args.obs_shape) + args.action_dim, hidden_dims=args.hidden_dims)
    critic2_backbone = MLP(input_dim=np.prod(args.obs_shape) + args.action_dim, hidden_dims=args.hidden_dims)
    # critic1_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims)
    # critic2_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims)
    # print('critic params', np.prod(args.obs_shape), args.hidden_dims, args.action_dim)
    # print('actor params', np.prod(args.next_obs_shape))
