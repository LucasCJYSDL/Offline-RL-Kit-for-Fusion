...
class D4RL_occ_dataset(torch.utils.data.Dataset):
    def __init__(self, args, device):
        self.args=args
        data = d4rl.qlearning_dataset(gym.make(args.env))()
        print("data keys", data.keys())
        self.device = device
        self.states = torch.from_numpy(data['observations'][:-1]).float().to(self.device)
        self.actions = torch.from_numpy(data['actions'][:-1]).float().to(self.device)
        self.next_states = torch.from_numpy(data['next_observations'][:-1]).float().to(self.device)
        self.next_actions = torch.from_numpy(data['actions'][1:]).float().to(self.device)
        reward = torch.from_numpy(data['rewards'][:-1]).view(-1, 1).float().to(self.device)
        self.is_finished = torch.from_numpy(data['terminals'][:-1]).view(-1, 1).float().to(self.device)
        self.timeouts = torch.from_numpy(data['timeouts'][:-1]).view(-1, 1).float().to(self.device)
    ...
    def __getitem__(self, index):
        data = {'s': self.states[index % self.len],
                ...
                'a_':self.next_actions[index % self.len],
...
class CustomDataset(Dataset):
    def __init__(self, obs, act, next_obs, rwd):
        super().__init__()
        ...
        self.rwd = torch.from_numpy(rwd).to(torch.float32)
    ...
    def __getitem__(self, idx):
        return self.obs[idx], self.act[idx], self.next_obs[idx], self.rwd[idx]     
        
class CustomDiffusionDataset(Dataset):
    def __init__(self, obs, act, next_obs, future_obs, next_act, rwd, done, next_done, idx, device = None):
        super().__init__()
        self.obs = obs.to(device)
        self.act = act.to(device)
        self.next_obs = next_obs.to(device)
        self.rwd = rwd.to(device)
        self.next_act = next_act.to(device)
        self.done = done.to(device)
        self.next_done = next_done.to(device)
        self.future_obs = future_obs.to(device)
    ...
    def __getitem__(self, index):
        ...
        data = {'s': self.obs[index % self.len],
                'a': self.act[index % self.len],
                'r': self.rwd[index % self.len],
                's_':self.next_obs[index % self.len],
                'a_':self.next_act[index % self.len],
                'd': self.done[index % self.len],
                'd_': self.next_done[index % self.len],
                'sf' : self.future_obs[index % self.len],
                'idx' : self.idx[index % self.len]
            }
        return data
